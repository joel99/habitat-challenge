...
ADD configs/ configs/
ADD ckpts/attn-e_auxed.85.pth ckpts/attn-e_auxed.85.pth
...
CMD ["/bin/bash", "-c", "source activate habitat && export PYTHONPATH=/evalai-remote-evaluation:$PYTHONPATH && export CHALLENGE_CONFIG_FILE=$TRACK_CONFIG_FILE && bash submit_aux.sh --model-path ckpts/attn-e_auxed.85.pth --config-path configs/attn-e_auxed.yaml"]
